...
#define ACS_712        34 // GPIO34 (ADC1_CHANNEL_6)
#define ON_OFF_RELAY   26
#define ALL_TIME_HIGH  35

const char* ssid = "Galaxy";
const char* password = "Alfarrel";
const char* mqtt_broker = "8bda2df24fea4d2c9aadeb89eedd2738.s1.eu.hivemq.cloud";
const int mqtt_port = 8883;
const char* mqtt_username = "hivemq.webclient.1749605718526";
const char* mqtt_password = "YU0lJBn9Ab%S.p<6,f2z";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "iot/perintah/relay_on_off") {
    if (message == "1") {
      onofflamp(true);
    } else if (message == "0") {
      onofflamp(false);
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("", mqtt_username, mqtt_password)) {
      ...
      client.subscribe("iot/perintah/relay_on_off");
    } else {
...
void setup() {
  ...
  if (validReadings == 0) {
    ...
    offset = total / (float)validReadings * 3.3 / 4095.0;
  }
  Serial.print("Offset voltage (no current): ");
  Serial.println(offset, 3);

  configTime(25200, 0, "pool.ntp.org", "time.nist.gov"); // 25200 = 7 jam (UTC+7)
...
void loop() {
  ...
  if (!relayState) {
    ...
    if (adc == 0) {
      ...
      current = (voltage - offset) / ACS712_SENSITIVITY;
